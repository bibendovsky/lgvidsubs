...
protected:
	...
	cThreadLock m_Lock;
private:
	HRESULT Call(DWORD dwParam, BOOL fBoostPriority)
	{
		...
		for(;;) {
			while(pkt2->size > 0) {
				...
				if (dec->sample_fmt != audio_src_fmt) {
					...
					reformat_ctx = av_audio_convert_alloc(AV_SAMPLE_FMT_S16, 1, dec->sample_fmt, 1, NULL, 0);
...
BOOL cLGVideoDecoder::Init(const char *filename)
{
	...
	if (FFmpeg::avcodec_enable_ir50dll)
	{
		...
		{
			struct _stat st;
			if (!m_pHostIface->GetConfigValue("ffmpeg_force_intern_indeo5", NULL, 0) && !_stat("ir50_32.dll", &st))
				initialized = 1;
			else
				initialized = -1;
		}
