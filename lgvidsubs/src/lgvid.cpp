...
namespace FFmpeg
{
	...
	BOOL Init(cLGVideoDecoder *pOuter_)
	{
		...
		INIT_FF_CALL (avcodec_register_all);
		// BBi
		...
		// BBi
		INIT_FF_CALL (avcodec_free_frame);
		// BBi
        ...
        INIT_FF_CALL (av_frame_get_best_effort_timestamp);
        INIT_FF_CALL (av_frame_get_channel_layout);
        INIT_FF_CALL (av_frame_get_channels);
        INIT_FF_CALL (av_frame_get_sample_rate);
...
fail:
	...
	// BBi
	{
		...
		(*ic_ptr)->pb = pb;
		ret = avformat_open_input (ic_ptr, filename, fmt, NULL);
		// BBi
	...
	int audio_decode_frame(double *pts_ptr)
	{
		...
		for(;;) {
			while(pkt2->size > 0) {
                ...
                int got_frame = 0;
                len1 = FFmpeg::avcodec_decode_audio4 (dec, audio_frame, &got_frame, pkt2);
                // BBi
                ...
                // BBi
                if (got_frame != 0) {
                    data_size = FFmpeg::av_samples_get_buffer_size (
                        NULL, audio_channels, audio_frame->nb_samples,
                        audio_format, 1);
                } else
                    data_size = 0;
                // BBi
                ...
                // BBi
                AVSampleFormat src_format = static_cast<AVSampleFormat> (
                    audio_frame->format);

                int src_channels =
                    FFmpeg::av_frame_get_channels (audio_frame);

                int64_t src_channel_layout =
                    FFmpeg::av_frame_get_channel_layout (audio_frame);

                if (src_channel_layout == 0) {
                    src_channel_layout =
                        FFmpeg::av_get_default_channel_layout (src_channels);
                }

                int src_sample_rate =
                    FFmpeg::av_frame_get_sample_rate (audio_frame);
                // BBi
...
//					}
                ...
                if ((src_format != swr_format) ||
                    (src_channels != swr_channels) ||
                    (src_channel_layout != swr_channel_layout) ||
                    (src_sample_rate != swr_sample_rate))
                {
                    if (swr_ctx != NULL)
                        FFmpeg::swr_free (&swr_ctx);

                    swr_ctx = FFmpeg::swr_alloc_set_opts (NULL,
                        audio_channel_layout, audio_format, audio_sample_rate,
                        src_channel_layout, src_format, src_sample_rate, 0, NULL);

                    if (swr_ctx == NULL) {
                        Warning (("Failed to allocate an audio resample context."));
                        break;
                    }

                    if (FFmpeg::swr_init (swr_ctx) < 0) {
                        Warning (("Failed to initialize an audio resample context."));
                        break;
                    }
                    // BBi
                    ...
                    swr_format = src_format;
                    swr_channels = src_channels;
                    swr_channel_layout = src_channel_layout;
                    swr_sample_rate = src_sample_rate;
                    // BBi
				}
...
//					data_size = len*2;
                ...
                if (swr_ctx != NULL) {
                    const uint8_t** in = const_cast<const uint8_t**> (
                        audio_frame->extended_data);

                    uint8_t* out[] = { audio_buf2, };
                    int out_count = sizeof (audio_buf2) / (audio_channels * audio_bps);
                    int len2 = FFmpeg::swr_convert (swr_ctx, out, out_count,
                        in, audio_frame->nb_samples);

                    if (len2 < 0) {
                        Warning (("Failed to convert audio samples."));
                        break;
                    }

                    if (len2 == out_count) {
                        Warning (("Audio resample buffer is probably too small."));
                        FFmpeg::swr_init (swr_ctx);
                        break;
                    }

                    audio_buf = audio_buf2;
                    data_size = len2 * audio_channels * audio_bps;
                // BBi
    ...
    virtual DWORD ThreadProc()
	{
		...
		for(;;) {
			...
			if(frameFinished) {
                ...
                pts_int = FFmpeg::av_frame_get_best_effort_timestamp (pFrame);
                // BBi
...
private:
    ...
    public:
        ...
        {
            return (mTime >= subtitle.time_begin) && (mTime <= subtitle.time_end);
        }
...
//				break;
            ...
            audio_channel_layout = FFmpeg::av_get_default_channel_layout (
                audio_channels);
